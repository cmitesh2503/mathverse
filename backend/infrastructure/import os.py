...
EMBEDDING_MODEL_NAME = os.getenv("MATHVERSE_EMBEDDING_MODEL", "text-embedding-004")
VERTEX_AI_LOCATION = (
...
def get_query_embedding(text: str) -> list[float]:
    ...
    if GEMINI_API_KEY:
        legacy_models = [
            EMBEDDING_MODEL_NAME if EMBEDDING_MODEL_NAME.startswith("models/") else f"models/{EMBEDDING_MODEL_NAME}",
        ...
        try:
            ...
            for model_name in legacy_models:
                try:
                    res = legacy_genai.embed_content(model=model_name, content=text)
                    if isinstance(res, dict) and "embedding" in res:
                        return res["embedding"]
                except Exception as e:
                    errors.append(f"Legacy {model_name}: {e}")
        except Exception as e:
            errors.append(f"Legacy init: {e}")
        ...
        try:
            ...
            for model_name in new_models:
                try:
                    ...
                    errors.append(f"New {model_name}: {e}")
        except Exception as e:
            errors.append(f"New init: {e}")

        print(f"All GenAI embeddings failed. Errors: {errors}")
        print("Bypassing Vertex AI to prevent application freeze. Returning empty embedding.")
        return [0.0] * 768
    ...
    try:
        ...
        print(f"Vertex AI embedding failed: {e}")
        return [0.0] * 768
...
def retrieve_context(
    ...
    **_: Any,
    ...
    The extra keyword arguments are accepted so all existing tutor call sites
    route through this Firestore-only implementation.
    """
    del exam_type, phase
