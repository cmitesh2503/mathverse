...
PDF_ROOT_DIR = BACKEND_DIR / "offline_assets" / "pdfs"
OUTPUT_FILE = BACKEND_DIR / "offline_assets" / "processed_content.txt"
    ...
    for page in reader.pages:
        ...
        if len(page_text) < MIN_EXTRACTED_TEXT_LEN:
            ocr_images = convert_from_path(
                str(file_path),
                first_page=len(page_texts) + 1,
                last_page=len(page_texts) + 1,
            )
            ocr_text = ""
