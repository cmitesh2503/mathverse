...
export type { WhiteboardPayload } from "../types/whiteboard";
...
export default function ClassWhiteboard({
  ...
  const boardSegments = useMemo(
    () => [
      ...
      whiteboard?.subtitle ||
        "Ava writes steps, equations, and graphs directly on the board as the class moves.",
      ...equations,
  ...
  const hasBoardContent = Boolean(graph || whiteboard?.problem || equations.length || chalkLines.length);
  ...
  return (
    ...
    >
      ...
      <div
        className={`mt-4 rounded-[1.35rem] border border-white/8 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.04),transparent_48%),linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))] ${
          isFocusStage ? "min-h-[540px] p-5" : "min-h-[360px] p-4"
        }`}
      >
        {hasBoardContent ? (
          <div className="space-y-5">
            {whiteboard?.problem && (
              <div
                className={`max-w-5xl rounded-[1rem] border border-amber-200/15 bg-amber-50/8 px-4 py-3 text-amber-50 ${
                  isFocusStage ? "text-base" : "text-sm"
                }`}
              >
                {revealedProblem || "Ava is writing the question on the board..."}
              </div>
            )}

            {equations.length > 0 && (
              <div
                className={`space-y-3 font-mono leading-7 text-[#fef3c7] ${
                  isFocusStage ? "text-[17px]" : "text-[15px]"
                }`}
              >
                {revealedEquations.length > 0 ? (
                  revealedEquations.map((equation) => (
                    <div key={equation} className="max-w-4xl rounded-xl bg-black/12 px-4 py-3">
                      {equation}
                    </div>
                  ))
                ) : (
                  <div className="max-w-4xl rounded-xl bg-black/12 px-4 py-3 text-emerald-50/55">
            ...
            {chalkLines.length > 0 ? (
              <div
                className={`space-y-3 leading-6 text-emerald-50/88 ${
                  isFocusStage ? "text-[15px]" : "text-sm"
                }`}
              >
                {revealedChalkLines.length > 0 ? (
                  revealedChalkLines.map((line) => (
                    <div key={line} className="flex gap-3 rounded-xl bg-black/10 px-4 py-3">
                      <span className="mt-[0.55rem] h-2 w-2 rounded-full bg-emerald-100/70" />
                      <span>{line}</span>
                    </div>
                  ))
                ) : (
                  <div className="max-w-4xl rounded-xl bg-black/10 px-4 py-3 text-emerald-50/55">
              ...
              !whiteboard?.problem && (
                <div
                  className={`max-w-4xl rounded-xl bg-black/10 px-4 py-3 text-emerald-100/72 ${
                    isFocusStage ? "text-base" : "text-sm"
                  }`}
                >
                  Ava will fill the board with short notes, equations, and examples as the class
                  continues.
                </div>
              )
            )}
            ...
            {graph && (
              <div className="overflow-hidden rounded-[1rem] border border-white/10 bg-black/12 p-2">
                <svg
                  viewBox={`0 0 ${BOARD_WIDTH} ${BOARD_HEIGHT}`}
                  className={`w-full rounded-[0.9rem] ${
                    isFocusStage ? "h-[300px] xl:h-[52vh]" : "h-[240px]"
                  }`}
          ...
          >
            Ava will write definitions, examples, equations, and graphs directly on this whiteboard
            as the lesson moves.
          </div>
        )}
      </div>
